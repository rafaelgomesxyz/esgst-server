...
class Utils {
  ...
  static validateParams(params, validator) {
    ...
    if (
      paramKeys.filter((paramKey) => !validatorKeys.includes(paramKey)).length >
      0
    ) {
    ...
    for (const validatorKey of validatorKeys) {
      ...
      if (
        ...
        throw new CustomError(
          `Invalid ${validatorKey} parameter. ${validation.message}`,
          400
        );
      }
      ...
      const intersection = paramKeys.filter((paramKey) =>
        validation.conflicts.includes(paramKey)
      );
