  ...
  static async _find(connection, req) {
    ...
    for (const type of Game.TYPES) {
      ...
      if (params[`${type}_ids`]) {
        conditions.push(
          `(${ids
            .map((id) => `g_tncv.${type}_id = ${connection.escape(id)}`)
            .join(' OR ')})`
        );
      }
    ...
    if (timestampRow) {
      result.last_update = Utils.formatDate(
  ...
  static async _insert(connection, req) {
    ...
    const check = (games) => {
      ...
      for (const id of ids) {
        ...
        games[id] = Math.trunc(
          new Date(`${games[id]}T00:00:00.000Z`).getTime() / 1e3
        );
      }
      return true;
    };
    ...
    for (const type of Game.TYPES) {
      ...
      for (const id of ids) {
        ...
        const element = html.querySelector(
          `.table__column__secondary-link[href*="${type}/${id}/"`
        );
        if (!element) {
          continue;
        }
        ...
        if (dateElement) {
          const dateRows = JSON.parse(
            dateElement.getAttribute('data-ui-tooltip')
          ).rows;
          const date = Math.trunc(
            new Date(
              `${dateRows[dateRows.length - 1].columns[1].name} UTC`
            ).getTime() / 1e3
          );
          if (row) {
            const effective_date = parseInt(row.effective_date);
        ...
        if (ncv.toRemove[type].length > 0) {
          await Pool.query(
            ...
            `
						...
						WHERE ${ncv.toRemove[type]
              .map((id) => `${type}_id = ${connection.escape(id)}`)
              .join(' OR ')}
					`
          );
        }
