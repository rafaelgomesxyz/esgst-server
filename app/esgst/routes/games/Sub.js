...
class Sub {
  ...
  static async get(connection, req, ids) {
    ...
    const params = Object.assign(
      {},
      { filters: req.query.filters || req.query.sub_filters || '' }
    );
    const validator = {
      filters: {
    ...
    for (const row of rows) {
      ...
      if (differenceInSeconds < 60 * 60 * 24 * 7) {
        ...
        if (Utils.isSet(columns.apps)) {
          sub.apps = row.apps
            ? row.apps.split(',').map((appId) => parseInt(appId))
            : [];
        }
        sub.last_update = Utils.formatDate(
  ...
  static async fetch(connection, subId) {
    ...
    const apiData = apiResponse.json[subId].success
      ? apiResponse.json[subId].data
      : null;
    if (!apiData) {
      return;
    }
    ...
    const sub = {
      ...
      removed: !storeResponse.url.match(
        new RegExp(`store\.steampowered\.com.*?\/sub\/${subId}`)
      ),
    ...
    const apps = apiData.apps
      ? apiData.apps.map((item) => parseInt(item.id))
      : [];
