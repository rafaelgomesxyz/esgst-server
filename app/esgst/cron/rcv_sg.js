  ...
  if (page == 0) {
    ...
    try {
      for (const type of Game.TYPES) {
        ...
        await Pool.query(
          connection,
          `UPDATE games__${type}_rcv SET found = FALSE`
        );
      }
  ...
  do {
    ...
    for (const element of elements) {
      ...
      const effectiveDate = Math.trunc(
        new Date(
          `${
            element.querySelector('.table__column--width-small').textContent
          } UTC`
        ).getTime() / 1e3
      );
        ...
        if (names[type].length > 0) {
          await Pool.query(
            ...
            `
              ...
              .map(
                (name) =>
                  `(${connection.escape(name.id)}, ${connection.escape(
                    name.name
                  )})`
              )
              .join(', ')}
  ...
  try {
    ...
    await Pool.query(
      ...
      `
			...
			VALUES ('rcv_last_update_from_sg', ${connection.escape(
        Math.trunc(Date.now() / 1e3)
      )})
